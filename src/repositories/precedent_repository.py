    ...
    def search_precedent_with_fallback(
        ...
        if original_query:
            ...
            if keywords:
                ...
                fallback_queries.append((keyword_query, "keyword_only"))

                # 긴 문구보다 단일 법리 키워드가 더 잘 잡히는 경우가 많아 개별 키워드도 시도
                for keyword in sorted(set(keywords), key=len, reverse=True):
                    fallback_queries.append((keyword, "single_keyword"))

                for fallback_query, strategy in fallback_queries:
                    logger.debug("Step D: %s | query=%r", strategy, fallback_query)

                    result = self._search_precedent_internal(
                        fallback_query, page, per_page, court, None, None, arguments  # 날짜 제한 없음
                    )
                    attempts.append({
                        "step": "D",
                    ...
                    if result.get("total", 0) > 0 and "error" not in result:
                        best_result = result
                        logger.debug("Step D succeeded | query=%r total=%d", fallback_query, result.get("total", 0))
                        break
    ...
    def _finalize_result(
        ...
        return {
            **normalized_response,
            ...
            "matched_query": best_result.get("query"),
            "query_plan": query_plan,
            "attempts": attempts,
            "best_result": best_result,
            "fallback_used": fallback_used,
