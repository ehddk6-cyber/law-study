...
class LawDetailRepository(BaseLawRepository):
    ...
    @staticmethod
    def _normalize_article_number(article_number: Optional[str]) -> str:
        if not article_number:
            return ""
        normalized = str(article_number).strip()
        normalized = normalized.replace(" ", "")
        normalized = normalized.replace("제", "").replace("조", "")
        normalized = normalized.replace("항", "").replace("호", "")
        return normalized

    @classmethod
    def _format_article_unit(cls, article_unit: dict) -> str:
        parts = []

        headline = article_unit.get("조문내용")
        if headline:
            parts.append(str(headline).strip())

        hang_list = article_unit.get("항")
        ...
        if isinstance(hang_list, list):
            for hang in hang_list:
                ...
                if hang_text:
                    parts.append(str(hang_text).strip())
                ...
                if isinstance(ho_list, list):
                    for ho in ho_list:
                        ho_text = ho.get("호내용")
                        if ho_text:
                            parts.append(str(ho_text).strip())
                        ...
                        if isinstance(mok_list, list):
                            for mok in mok_list:
                                ...
                                if mok_text:
                                    parts.append(str(mok_text).strip())
    ...
    def get_single_article(self, law_id: str, article_number: str, hang: Optional[str] = None, 
        ...
        try:
            ...
            article_units = self.find_nested_value(detail_data, "조문단위")
            normalized_article_number = self._normalize_article_number(article_number)
            ...
            if isinstance(article_units, list):
                for article_unit in article_units:
                    unit_number = self._normalize_article_number(article_unit.get("조문번호"))
                    if unit_number == normalized_article_number and article_unit.get("조문여부") == "조문":
                        article_title = article_unit.get("조문제목") or article_number
                        article_content = self._format_article_unit(article_unit)
                        result = {
                            "law_id": law_id,
                            ...
                            "api_url": detail_response.url,
                            "source": "law_detail_embedded_article",
                        }
                        if not article_content:
                            result["note"] = "법령 상세 응답에서 조문 항목은 찾았지만 본문이 비어 있습니다."
                        return result
            
            # 시행일자(efYd) 추출
            ef_yd = self.find_nested_value(
                detail_data,
                "시행일자",
                "efYd",
                "시행일",
            ...
            try:
                ...
                if isinstance(data, dict):
                    article_content = self.find_nested_value(
                        data,
                        "조문내용",
                        "articleContent",
                        "내용",
                    ...
                    article_title = self.find_nested_value(
                        data,
                        "조문제목",
                        "articleTitle",
                        "제목",
