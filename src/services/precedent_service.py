...
class PrecedentService:
    ...
    def _infer_issue_type(query: Optional[str]) -> Optional[str]:
        if not query:
            return None
        # 헌법·행정법 키워드
        if any(keyword in query for keyword in ["무효", "명백", "중대", "하자", "취소"]):
            return "행정행위무효"
        if any(keyword in query for keyword in ["원고적격", "피고적격", "소송요건"]):
            return "소송요건"
        if any(keyword in query for keyword in ["처분성", "공권력성"]):
            return "처분성"
        if any(keyword in query for keyword in ["재량", "남용", "일탈"]):
            return "재량권남용"
        if any(keyword in query for keyword in ["정보공개", "비공개"]):
            return "정보공개"
        if any(keyword in query for keyword in ["개인정보", "유출"]):
            return "개인정보"
        # 근로자성 관련
